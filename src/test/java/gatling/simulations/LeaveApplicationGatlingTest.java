 ...
 */
public class LeaveApplicationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all leaveApplications").get("/api/leave-applications").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new leaveApplication")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"reason\": \"SAMPLE_TEXT\"" +
                                ", \"submissionDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"status\": \"PENDING\"" +
                                ", \"rejectionReason\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_leaveApplication_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created leaveApplication").get("${new_leaveApplication_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created leaveApplication").delete("${new_leaveApplication_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LeaveApplication entity").exec(scn);
